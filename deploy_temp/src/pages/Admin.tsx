...
import { 
  ...
  BarChart3, Users, Zap, Upload, Plus, Trash2, Database, FileSpreadsheet, PenTool, UserPlus, Download
} from 'lucide-react';
      ...
      <div className="max-w-[1600px] mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm">{t('admin.header.subtitle')}</p>
          </div>
          <button onClick={fetchLeads} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-300 hover:bg-white/10 transition-all disabled:opacity-50 cursor-pointer">
            ...
            <motion.div className="bg-[#0D0F16] border border-white/10 rounded-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl">
               ...
               <div className="flex-1 overflow-y-auto p-8 space-y-6">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="space-y-4">
                     <div><label className="text-[10px] uppercase tracking-widest text-gray-500 mb-2 block">Categoría</label><select value={editingPost.category} onChange={e => setEditingPost({...editingPost, category: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white outline-none focus:border-indigo-500"><option value="Seguridad" className="bg-[#0D0F16]">Seguridad</option><option value="Estrategia" className="bg-[#0D0F16]">Estrategia</option><option value="Tecnología" className="bg-[#0D0F16]">Tecnología</option><option value="Diplomacia" className="bg-[#0D0F16]">Diplomacia</option></select></div>
