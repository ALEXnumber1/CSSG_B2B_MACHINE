...
const services = [
  { icon: <Crosshair className="w-6 h-6" />, accent: 'sky', bg: '/svc_perimetral.png' },
  { icon: <Shield className="w-6 h-6" />, accent: 'sky', bg: '/diplomatic_security.png' },
  { icon: <Activity className="w-6 h-6" />, accent: 'sky', bg: '/svc_auditoria.png' },
  { icon: <Users className="w-6 h-6" />, accent: 'sky', bg: '/svc_ejecutivos.png' },
  { icon: <Zap className="w-6 h-6" />, accent: 'emerald', bg: '/svc_reaccion_motorizada.png' },
  { icon: <Truck className="w-6 h-6" />, accent: 'emerald', bg: '/svc_custodia.png' },
  { icon: <Activity className="w-6 h-6" />, accent: 'emerald', bg: '/ambulancia.png' },
  { icon: <GraduationCap className="w-6 h-6" />, accent: 'amber', bg: '/svc_capacitacion.png' },
  { icon: <MessageSquare className="w-6 h-6" />, accent: 'sky', bg: '/svc_licitaciones.png' },
];
          ...
          {services.map((svc, i) => {
            ...
            return (
              <motion.div 
                ...
                className={`group relative overflow-hidden border border-white/[0.08] rounded-3xl ${colors.hoverBorder} transition-all duration-500 min-h-[300px] shadow-2xl ${isMain ? 'ring-2 ring-orange-500/50 scale-105 z-20' : ''}`}
              >
                <img 
                  src={svc.bg} 
                  alt="" 
                ...
                <div className="relative z-10 p-8 h-full flex flex-col justify-end">
                  ...
                  <h3 className={`text-xl font-black mb-3 tracking-tight leading-tight uppercase ${isMain ? 'text-black' : 'text-white'}`}>
                    {isMain ? t('services.s2_title_clean') : t(`services.s${i + 1}_title`)}
                  </h3>
          ...
          <div className="relative z-10 p-8 md:p-14 flex flex-col lg:flex-row items-center justify-between gap-10">
            <div className="max-w-2xl text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 mb-6">
                ...
                <span className="text-[10px] font-black text-sky-400 uppercase tracking-widest">Consultoría de Alto Nivel</span>
              </div>
              <h3 className="text-3xl md:text-4xl font-black text-white mb-6 tracking-tighter uppercase italic leading-[0.9]">
                ¿Su proyecto exige un blindaje <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-sky-100">operativo sin fisuras?</span>
              </h3>
              <p className="text-gray-400 text-sm md:text-lg leading-relaxed font-light max-w-xl">
                Agende una sesión técnica de 15 minutos. Analizaremos su requerimiento específico y diseñaremos la hoja de ruta para una seguridad de estándar internacional.
              </p>
            </div>
            
            <div className="shrink-0">
              <button 
                onClick={() => window.open('https://calendar.app.google/jGP8yq3BCM5Rp525A', '_blank')}
                className="group relative flex flex-col items-center gap-3 px-14 py-10 bg-sky-700 hover:bg-sky-600 text-white rounded-[2rem] font-black transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 active:translate-y-0"
              >
                <div className="flex items-center gap-5 uppercase tracking-[0.3em] text-sm md:text-base">
                  <span>Programar Sesión en Calendar</span>
                  <ArrowRight className="w-6 h-6 group-hover:translate-x-3 transition-transform" />
                </div>
                <span className="text-xs md:text-sm text-orange-400/90 normal-case tracking-normal font-bold">
                  Y obten el folleto de la solución, explicado por un experto.
                </span>
              ...
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest text-center mt-4">
                Últimos cupos disponibles para esta semana
              </p>
            </div>
          </div>
        </motion.div>
