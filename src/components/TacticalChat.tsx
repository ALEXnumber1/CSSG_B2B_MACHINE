...
const TacticalChat: React.FC = () => {
  ...
  return (
    <div className="hidden md:block fixed bottom-40 lg:bottom-48 right-6 z-[100]">
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            <div className="absolute right-20 top-1/2 -translate-y-1/2 pointer-events-none">
              ...
              >
                ...
                <div className="flex flex-col pr-2">
                  ...
                  <span className="text-[11px] text-white font-medium leading-tight">{t('chat.greeting')}</span>
                </div>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-[450px] h-[600px] max-h-[85vh] bg-[#0D0F16] border border-sky-500/30 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden backdrop-blur-2xl"
            >
            {/* Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] text-emerald-400 font-black uppercase">{t('chat.status_online')}</span>
                  </div>
                </div>
              </div>
                ...
                >
                  <div className={`max-w-[80%] flex gap-3 ${msg.type === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex flex-col gap-1 ${msg.type === 'user' ? 'items-end' : 'items-start'}`}>
                      {msg.type === 'bot' && (
                        <span className="text-[9px] text-sky-400/80 font-bold ml-1 uppercase tracking-wider">{t('chat.agent_role')}</span>
                      )}
            ...
            {/* Input Area */}
            <div className="p-4 pb-6 border-t border-white/5 bg-black/40">
              ...
              <div className="mt-4 flex items-center justify-center gap-4 text-[8px] text-gray-600 font-black uppercase tracking-widest">
                <div className="flex items-center gap-1"><Zap size={10} /> {t('chat.status_instant')}</div>
                <div className="flex items-center gap-1"><Shield size={10} /> {t('chat.status_secure')}</div>
              </div>
            </div>
          </motion.div>
        </div>
