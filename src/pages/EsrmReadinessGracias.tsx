...
export default function EsrmReadinessGracias() {
  const location = useLocation();
  const navigate = useNavigate();
  const state = (location.state || {}) as LocationState;

  useEffect(() => {
    // Acceso directo sin pasar por el formulario — no hay nada que confirmar
    if (!state.correo) {
      navigate('/esrm-readiness', { replace: true });
      return;
    }
    ...
    <div style={{ background: NAVY, color: WHITE, fontFamily: "'Inter', sans-serif", lineHeight: 1.6, minHeight: '100vh' }}>
      ...
      <main className="relative flex flex-col items-center justify-center px-6 overflow-hidden" style={{ minHeight: '78vh' }}>
        ...
        <motion.div className="relative max-w-lg w-full text-center py-16" initial="hidden" animate="visible" variants={fadeUp}>
          ...
          <p style={{ color: MUTED }}>
            {state.duplicate
              ? <>Ya contamos con su información en nuestro sistema. Nuestro equipo de compliance le enviará el expediente completo a <b style={{ color: WHITE }}>{state.correo}</b> en menos de 24 horas hábiles.</>
              : <>Nuestro equipo de compliance le enviará el expediente completo — certificaciones, matrices ISO 31000 y evidencia operativa — a <b style={{ color: WHITE }}>{state.correo}</b> en menos de 24 horas hábiles.</>}
          </p>
          ...
          <div className="mt-8 pt-8 text-left" style={{ borderTop: `1px solid ${BORDER}` }}>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                ...
                <span className="text-sm" style={{ color: MUTED }}>Si su proceso requiere NDA, RFI o RFP, respóndanos indicándolo y lo incluimos en el primer envío.</span>
              </li>
              <li className="flex items-start gap-3">
                <Lock className="w-4 h-4 shrink-0 mt-0.5" style={{ color: GOLD }} />
                <span className="text-sm" style={{ color: MUTED }}>Toda la información se maneja bajo protocolo de confidencialidad.</span>
              </li>
            </ul>
          </div>
