...
import { Check, ArrowRight, Lock, Mail, ShieldCheck, FileCheck, Phone } from 'lucide-react';
...
export default function InvestorSecurityThankYou() {
  const location = useLocation();
  const navigate = useNavigate();
  const state = (location.state || {}) as LocationState;
  const isDueDiligence = state.motivo === 'due_diligence';

  useEffect(() => {
    if (!state.correo) {
      navigate('/en/executive-protection-venezuela-investors', { replace: true });
      return;
    }
        ...
        <motion.div className="relative max-w-lg w-full text-center py-16" initial="hidden" animate="visible" variants={fadeUp}>
          ...
          <p className="text-gray-400">
            {state.duplicate ? (
              ...
              <>A senior consultant is scoping your due diligence / vetting study and will contact you at <b className="text-white">{state.correo}</b> within 12 business hours.</>
            ) : (
              <>A senior consultant will review your itinerary and contact you at <b className="text-white">{state.correo}</b> within 12 business hours to schedule your pre-travel risk assessment.</>
            )}
          </p>
            ...
            <ul className="space-y-3">
              ...
              {isDueDiligence ? (
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-gray-400">Share the name of the counterparty or local partner in a reply — it lets us begin the vetting research before your call.</span>
                </li>
              ) : (
                <li className="flex items-start gap-3">
                  <ShieldCheck className="w-4 h-4 shrink-0 mt-0.5" style={{ color: GOLD }} />
                  <span className="text-sm text-gray-400">Share your travel dates and destinations in a reply — it lets us start the preventive intelligence study before your call.</span>
                </li>
              )}
              <li className="flex items-start gap-3">
