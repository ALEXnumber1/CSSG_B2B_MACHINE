...
export default function DiplomaticSecurityThankYou() {
  ...
  useEffect(() => {
    if (!state.correo) {
      navigate('/en/diplomatic-security', { replace: true });
      return;
    }
        ...
        <motion.div className="relative max-w-lg w-full text-center py-16" initial="hidden" animate="visible" variants={fadeUp}>
          ...
          <p className="text-gray-400">
            {state.duplicate ? (
              ...
              <>A senior consultant is preparing our tender documentation — including ISO 9001:2015 certification and DIGESERVISP authorization — and will contact you at <b className="text-white">{state.correo}</b> within 12 business hours.</>
            ) : (
              <>A senior consultant will review your request and contact you at <b className="text-white">{state.correo}</b> within 12 business hours to schedule your Technical Site Assessment (levantamiento técnico).</>
            )}
          </p>
          ...
          <div className="mt-6">
            ...
            >
              Schedule My Technical Site Assessment <ArrowRight className="w-3.5 h-3.5" />
            </a>
            ...
            <ul className="space-y-3">
              ...
              {isTender ? (
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-gray-400">Share your submission deadline in a reply — we will return the required certifications and technical proposal ahead of your tender's cutoff.</span>
                </li>
              ) : (
                <li className="flex items-start gap-3">
                  <ShieldCheck className="w-4 h-4 shrink-0 mt-0.5" style={{ color: GOLD }} />
                  <span className="text-sm text-gray-400">Already have a provider in place? Mention it in your first call — we can start with an audit of your current scheme instead of proposing a new one.</span>
                </li>
              )}
              <li className="flex items-start gap-3">
